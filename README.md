...
#### Класс Api
Содержэит в себе базовую логику отправки запросов. В конструктор передается базовый адрес сервера и опциональный объект с заголовками запросов.
Методы:
...
- _cards: IProductItem[] - массив объектов карточек продуктов
- _preview: string | null - id карточки, выбранной для просмотра в модальном окне

Методы: 
- getCard(CardId: string): IProductItem - возвращает карточку по ёё id
...
#### Класс Modal
Реализует модальное окно. Так же предоставляет методы `open` и `close` для управления отображением модального окна.Устанавливвет слушатель для закрытия модального окна на клик в оверлей и енопку-крестик.
- constructor(protected container: HTMLElement) Конструктор принимает контент модального окна и экземпляр класса `EventEmitter` для возможности инициации событий.

Поля класса
- content: HTMLElement - элемент модального окна
- events: IEvants - брокер событий
